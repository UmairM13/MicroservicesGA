    ...
    def solve(self):
        ...
        for generation in range(0, Ng):
            ...
            for count in range(Ne, Nc, 2):
                ...
                if(success):
                    Nm += 1
                    # Only count as a successful improvement if fitness values are comparable/non-None.
                    if (child1.fitness is not None) and (old_fitness is None or child1.fitness > old_fitness):
                        phi = phi + 1
                ...
                if(success):
                    Nm += 1
                    # Only count as a successful improvement if fitness values are comparable/non-None.
                    if (child2.fitness is not None) and (old_fitness is None or child2.fitness > old_fitness):
                        phi = phi + 1
...
s = Sudoku()
s.load(os.path.join(script_dir, "puzzle_mild.txt"))
solution = s.solve()
if(solution):
